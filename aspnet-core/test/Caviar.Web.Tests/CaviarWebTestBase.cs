...
{
    public abstract class CaviarWebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
    {
        protected static readonly Lazy<string> ContentRootFolder;

        static CaviarWebTestBase()
        {
        ...
        protected override IWebHostBuilder CreateWebHostBuilder()
        {
            return base
                ...
                .UseSetting(WebHostDefaults.ApplicationKey, typeof(CaviarWebMvcModule).Assembly.FullName);
        }
        ...
        protected void UsingDbContext(Action<CaviarDbContext> action)
        {
            using (var context = IocManager.Resolve<CaviarDbContext>())
            {
                action(context);
                context.SaveChanges();
            }
        ...
        protected T UsingDbContext<T>(Func<CaviarDbContext, T> func)
        {
            T result;
        ...
        protected async Task UsingDbContextAsync(Func<CaviarDbContext, Task> action)
        {
            using (var context = IocManager.Resolve<CaviarDbContext>())
            {
        ...
        protected async Task<T> UsingDbContextAsync<T>(Func<CaviarDbContext, Task<T>> func)
        {
            T result;
